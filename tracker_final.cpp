...
struct group_info
{
	...
	//vector<>
	unordered_map<string,vector<pair<string,pair<string,int>>>> fileowners;//UPLOAD/DOWNLOAD
...
int upload_file(threads_info *p)
{
	...
	// //CHECK IF THE UPLOADER IS A GROUP MEMBER OR NOT
	 ...
	 for (auto it = gi.members.begin(); it != gi.members.end(); ++it) 
     {
    	...
    	{  		
    		listenip=(*it).second.first;
    		cout<<" ip "<<(*it).second.first;
    		cout<<"listen ip "<<listenip<<endl;
    		listenport=(*it).second.second;
    		cout<<"port "<<(*it).second.second;;
	...
	//Entering fileowners
	vector<pair<string,pair<string,int>>> li;
	pair<string,int>p3(listenip,listenport);
	cout<<"listen ip and port are <<"<<listenport<<listenip<<endl;
	pair<string,pair<string,int>>p4(username,p3);
	//li.push_back(p4);
	if(gi.fileowners.find(filename)!=gi.fileowners.end())
		li=gi.fileowners[filename];
...
//get the user id of the user calling create group
    ...
    { 
    	 cout << "Element  found at position : "<<endl ; 
    	 cout<<"itr "<<(*itr).lport<<"   "<<(*itr).lip<<endl;
   ...
   //  cout<<"checkingt  he owner info"<<endl;
   //  vector<pair<string,pair<string,int>>> temporar=gi_temp.fileowners["newfile"];
  	// cout<<"size ="<<temporar.size();
   //  cout<<temporar[0].second.second<<"  "<<temporar[0].second.first<<endl;
...
//get the user id of the user calling create group
	...
	string username=active_user[port];
	gi=group_map[grpid];
	...
	vector<pair<string,pair<string,int>>> temp_owners;
	temp_owners= gi.fileowners[filename];

	int no_of_owners=temp_owners.size();

	sentbytes=send(from_client,&no_of_owners,sizeof(no_of_owners),0);	
	
	int start=0;
	while(start<no_of_owners)
	{
		memset(buffer,'\0',512);
		readbytes=recv(from_client,buffer,512,0);
		cout<<"user is "<<temp_owners[start].first<<endl;
		string ip=temp_owners[start].second.first;
		cout<<"sender ip ="<<ip<<endl;
		strcpy(buffer,ip.c_str());
		cout<<endl<<"ending ip"<<buffer<<endl;
		sentbytes=send(from_client,buffer,512,0);
		memset(buffer,'\0',512);
		readbytes=recv(from_client,buffer,512,0);
		
		int port_num=temp_owners[start].second.second;
		cout<<"port is "<<port_num<<"   "<<temp_owners[start].second.second<<endl;
		sentbytes=send(from_client,&port_num,sizeof(port_num),0);	
	...
	//send the sha and the filesize
		memset(buffer,'\0',512);
		readbytes=recv(from_client,buffer,512,0);

		pair<string,int> fsz;
		fsz=gi.filenames[filename];
		int f=fsz.second;
		sentbytes=send(from_client,&f,sizeof(f),0);
		memset(buffer,'\0',512);
		readbytes=recv(from_client,buffer,512,0);
		string hash=fsz.first;

		//send sha length
		int sha_len=hash.length();
		send(from_client,&sha_len,sizeof(sha_len),0);
				//cout<<"filesize sent";
		readbytes=recv(from_client,buffer,512,0);
		memset(buffer,'\0',512);
		...
		{ 	//no_of_times++;
			...
			string temp=hash.substr(start_index,20);
			strcpy(send_sha,temp.c_str());
			cout<<"sending "<<send_sha<<endl;
			start_index=start_index+20;
			send(from_client,send_sha,sizeof(string(send_sha)),0);
			memset(send_sha,'\0',512);
			sha_len=sha_len-20;
			readbytes=recv(from_client,buffer,512,0);
